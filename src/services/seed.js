...
const { createApplication, updateStatus } = require('./applications');
...
async function seedIfEmpty() {
  const { total } = await getStore().list({ limit: 1 });
  if (total > 0) return 0;
  return seedDemoData();
}

module.exports = { seedDemoData, seedIfEmpty, SAMPLES };
